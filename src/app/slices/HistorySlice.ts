    ...
    reducers: {
        addToRecentlySearched: (state, action) => {
            if (action.payload) {
                ...
                const historyObj = {name: action.payload.name, id: action.payload.id, image: action.payload.image};
                if (state.recentlySearched ) {
                    const alreadySearched = state.recentlySearched.find((pokemon) => pokemon.id === historyObj.id);
                    if (!alreadySearched) {
                        ...
                        const foundIndex = state.recentlySearched.findIndex((pokemon) => pokemon.id === historyObj.id);
